...
def table(s=0):
    ...
    c.setFont(FontsHeader[random.randrange(0,len(FontsHeader))],font)
...
def tabletext(coordinates,tablehor,tablevert):
    ...
    for i in coordinates:
            ...
            if i[0][1]==(int(tablevert[-2])):
                ...
                header.setFont(FontsHeader[random.randrange(0,len(FontsHeader))],font)
                header.textLine("Header")
                c.drawText(header)
                c.restoreState()
                continue
            c.setFillColorRGB(r,g,b)
            header.setFont(Fonts[random.randrange(0,len(Fonts))],font)
...
def text():
    ...
    for i in range(x):
        ...
        b=random.random()
        c.setFillColorRGB(r,g,b)
        c.setFont(Fonts[random.randrange(0,len(Fonts))],random.randrange(10,13))
        c.drawString(0,cursor,"SomeRandomString, I know it's boring")
        
        c.restoreState()
        cursor+=random.randrange(10,13)
    return(x)
...
while(n<=numberdoc):    
    ...
    for i in range(5):
        ...
        if(cursor>=680):
            c.setFont(FontsHeader[random.randrange(0,len(FontsHeader))],20)
        ...
        if p==4:
            t+=1
            f=open(r"/home/thedamnchillguy/Desktop/Table Recognition/pdf/"+str(n)+".txt","a+")
            f.write(f"\nCoordinates:\nTable {t}")
            for item in table():
                f.write(f"\n{item}")
    ...
    c.save()
    if t==0:
        os.remove(r"/home/thedamnchillguy/Desktop/Table Recognition/pdf/"+str(n)+".pdf")
        tabledoc-=1
    n+=1
